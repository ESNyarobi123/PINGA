...
class Dashboard extends Component
{
    ...
    private function getStats(): array
    {
        ...
        return [
            ...
            'msingi_active' => Subscription::whereHas('plan', fn($q) => $q->where('slug', 'msingi'))
                ->active()->count(),
            'kawaida_active' => Subscription::whereHas('plan', fn($q) => $q->where('slug', 'kawaida'))
                ->active()->count(),
            'bora_active' => Subscription::whereHas('plan', fn($q) => $q->where('slug', 'bora'))
                ->active()->count(),
    ...
    private function getActivityFeed(): array
    {
        ...
        Subscription::with(['plan', 'user'])->latest('created_at')
            ->limit(10)
            ->get()
            ->each(fn($sub) => $activities->push([
                ...
                'description' => "{$sub->plan?->name} - {$sub->user?->name}",
