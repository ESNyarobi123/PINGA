<div>
    ...
    <div class="space-y-4">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                            ...
                            <span class="flex items-center gap-1">
                                ...
                                TZS {{ number_format($job->payment?->employer_amount ?? $job->budget_min ?? 0) }}
                            </span>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <button onclick="navigator.clipboard.writeText('{{ $job->completion_code }}')" class="p-2 text-zinc-500 hover:text-emerald-600 transition-colors" title="{{ __('messages.codes.copy') }}">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                @if($job->isOnCodeHold())
                                    ...
                                    @if(!$job->hasExtendedHold())
                                        <button wire:click="openExtendForm({{ $job->id }})"
                                    ...
                                    @endif
                                @else
                                    <button wire:click="holdCode({{ $job->id }})" 
                                            wire:confirm="Una uhakika unataka kushikilia malipo kwa masaa 3?"
                ...
                @else
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            ...
                            <button wire:click="generateCode({{ $job->id }})" 
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-12 text-center">
            <div class="w-20 h-20 mx-auto mb-4 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
                <svg class="w-10 h-10 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                </svg>
            </div>
            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white mb-2">{{ __('messages.codes.no_jobs') }}</h3>
            <p class="text-zinc-500 dark:text-zinc-400 mb-4 max-w-md mx-auto">
                {{ __('messages.codes.no_jobs_desc') }}
            </p>
    ...
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" wire:click="closeExtendForm">
        <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-md w-full p-6" wire:click.stop>
            <h3 class="text-lg font-bold text-zinc-900 dark:text-white mb-2">Ongeza Muda wa Tathmini</h3>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4">Andika sababu ya kuongeza muda kwa masaa 3 zaidi. Hii itaonekana kwa winga.</p>
            <form wire:submit="extendHold">
                <textarea wire:model="holdComment" rows="3" placeholder="Andika sababu ya kuongeza muda..." class="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-4 py-2 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 mb-2"></textarea>
