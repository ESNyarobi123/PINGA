            ...
            @if(count($myActiveJobs) > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($myActiveJobs as $jobOption)
                        <button wire:click="selectJob({{ $jobOption['id'] }})"
                            ...
                            <p class="text-xs text-zinc-500 mt-2">{{ __('messages.weka_code.click_to_select') }}</p>
                        </button>
                    @endforeach
                </div>
            @else
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6 space-y-4">
            ...
            @else
                <div class="text-center text-zinc-500">
                    <p>{{ __('messages.weka_code.select_job_prompt') }}</p>
                </div>
            @endif
        </div>
    </div>
